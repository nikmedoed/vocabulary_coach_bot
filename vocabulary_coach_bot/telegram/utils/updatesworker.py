...
def get_handled_updates_list(dp: Dispatcher) -> list:
    ...
    AVAILABLE_HANDLERS = (
        ...
        "poll_handlers", "pre_checkout_query_handlers", "shipping_query_handlers", "errors_handlers"
    )
    # AVAILABLE_UPDATES = tuple(item.replace("_handlers", "") for item in AVAILABLE_HANDLERS)
    return [item.replace("_handlers", "") for item in AVAILABLE_HANDLERS

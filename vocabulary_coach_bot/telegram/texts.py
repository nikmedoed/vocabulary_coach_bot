...
INSTRUCTION_LINK = 'https://github.com/nikmedoed/vocabulary_coach_bot/blob/main/docs/deploy_table.md'
...
SETUP_INSTRUCTION = ("Нужно\n"
                     f"👉 <a href='{INSTRUCTION_LINK}'>повторить действия из инструкции</a>\n"
                     "👉 перенести в новую таблицу накопленные данные\n"
                     f"👉 установить новую ссылку в /settings")
...
class Start(EnumContent):
    ...
    database_no_link = ("<b>Из базы пропала ссылку на вашу таблицу.</b>\n\n"
                        "Это может быть сбой, или обновления движка таблицы.\n\n" +
                        SETUP_INSTRUCTION)
    database_old_version = ("<b>Версия вашей таблицы устарела</b>\n\n"
                            "Движок таблицы должен обновляться автоматически, "
                            "но в вашем случае произошёл сбой.\n\n" +
                            SETUP_INSTRUCTION)
...
class Settings(EnumContent):
    ...
    sheet_url_message = (f"👉 Сейчас мне нужна ссылка на настроенную таблицу.\n\n"
                         f"<a href='{INSTRUCTION_LINK}'>Инструкция, как получить ссылку</a>")
